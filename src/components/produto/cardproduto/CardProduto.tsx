...
function CardProduto({ produtos, loading }: CardProdutoProps) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-[#a717eb]/20 overflow-hidden">
            {/* Tabela — visível a partir de lg, com rolagem horizontal de segurança */}
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            produtos.map((produto) => (
                                ...
                                >
                                    <td className="py-3 px-6 text-base text-gray-800 font-medium">
            ...
            {/* Lista em cards — visível abaixo de lg (tablet/celular) */}
            <div className="lg:hidden divide-y divide-gray-100">
                {loading ? (
                    ...
                    produtos.map((produto) => (
                        <div key={produto.id} className="p-4 flex flex-col gap-2">
                            <div className="flex justify-between items-start gap-2">
                                <div className="min-w-0">
                                    <p className="text-base font-medium text-gray-800 truncate">{produto.nome}</p>
                                    <p className="text-sm text-gray-500 truncate">
                                        {produto.usuario?.nome || 'Usuário não atribuído'}
                                    </p>
                                </div>
                                <span className={`text-xs font-medium px-2 py-1 rounded-full whitespace-nowrap ${STATUS_STYLES[produto.status] ?? 'bg-gray-100 text-gray-700'
                                    }`}>
                                    {produto.status}
                                </span>
                            </div>
                            ...
                            <div className="flex justify-between items-center text-sm text-gray-600">
                                <span>R$ {produto.valorDebito.toFixed(2).replace('.', ',')}</span>
                                <span className="text-gray-500">{produto.categoria?.nome || 'Sem categoria'}</span>
                            </div>
